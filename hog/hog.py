...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    # roll dice for num_rolls times
    roll_res = []
    for i in range(num_rolls):
        roll_res.append(dice())

    # if any of outcomes is 1, return 1; else return sum of outcomes
    if 1 in roll_res:
        return 1
    else:
        return sum(roll_res)
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        # when num_rolls == 0, pig tail
        return tail_points(opponent_score)
    else:
        # when num_rolls is not 0, roll dice
...
def perfect_square(score):
    """Return the judgement that whether the score is a perfect square or not.
    ...
    """
    base = sqrt(score)
    return base.is_integer()

def next_perfect_square(score):
    """Return next perfect squre of the score, when the score is a perfect square.

    >>> next_perfect_square(36)
    49
    >>> next_perfect_square(35)
    'AssertionError: Score should be a perfect square'
    """
    assert perfect_square(score)==True, 'Score should be a perfect square'
    # return next perfect square
    base = sqrt(score)
    return pow(int(base)+1, 2)
# END PROBLEM 4
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal: 
        # anyone reach goal points, end the game
        if who == 0:
            # player0's turn
            # get number of dice player0 choose to roll
            num_rolls = strategy0(score0, score1)

            # get updated score of player0
